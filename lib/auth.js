...
const { APP_SECRET, TOKEN_EXPIRY } = require('../environment');
...
internals.createAccessToken = function () {
  ...
  return async function (userId) {
    ...
    const token = await Jwt.sign({ _id: userId }, APP_SECRET, { expiresIn: TOKEN_EXPIRY });
...
internals.validatesFormDataContentTypeOnPost = (request, reply) => {
  ...
  if (DELETE_POST.includes(request.method)){
    if (request.headers['content-type'] !== URLENCODED) {
      throw Boom.badRequest('Request format incorrect');
    }
  }
