...
exports.create = {
  ...
  async handler(request, h) {
    ...
    let err, encryptedPassword, result, tokens;
    ...
    if (err && err.code === 11000) return Boom.badRequest('User already exists.');
    if (err) return Boom.internal();
...
exports.me = {
  ...
  async handler(request, h) {
    ...
    [err, user] = await to(db.collection('User').findOne(
      { _id: ObjectID(userId) },
